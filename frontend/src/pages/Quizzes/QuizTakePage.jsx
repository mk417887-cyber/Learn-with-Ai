  ...
  return (
    <div className="max-w-4xl-mx-auto">
      ...
      <div className="bg-white/80 backdrop-blur-xl border-2 border-slate-200 rounded-2xl shadow-xl shadow-slate-200/50 p-6">
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-xl mb-4  bg-linear-to-r from-emerald-50 to-teal-50 border border-emerald-100">
        ...
        <div className="space-y-3">
          {currentQuestion.options.map((option, index) => {
            ...
            return (
              <label
                key={index}
                className={`group relative flex items-center p-3 border-2 rounded-xl cursor-pointer transition-colors duration-200 ${isSelected
                  ? 'border-emerald-500 bg-emerald-50 shadow-lg shadow-emerald-500/10'
                  : 'border-slate-200 bg-slate-50/50 hover:border-slate-300 hover:bg-white hover:shadow-lg hover:shadow-slate-200/10'
                  }`}
              >
                <input
                  type="radio"
                ...
                {isSelected && (
                  <CheckCircle2
                    className="ml-auto w-4 h-5 text-emerald-500"
                    strokeWidth={2.5}
                  />
                )}
              </label>
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        {currentQuestionIndex === quiz.questions.length - 1 ? (
          <button
            ...
            disabled={submitting}
            className="group relative px-8 h-12 bg-linear-to-r from-emerald-500  to-teal-500 hover:to-teal-600 rounded-full flex items-center justify-center gap-4 text-white font-semibold transition-colors duration-200"
          >
            ...
            <div className="absolute inset-0 bg-linear-to-r from-emerald-500 to-teal-500  -translate-x-full rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-200" />
          </button>
        ) : (
          <Button
      ...
      {/* Question Indicators */}
      <div className="flex items-center gap-2 mt-0 justify-center flex-wrap">
