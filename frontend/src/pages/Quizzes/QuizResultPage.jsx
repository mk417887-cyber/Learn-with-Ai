    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="mb-6">
        <Link
          ...
          className="group inline-flex items-center gap-2 text-sm font-medium text-slate-600 hover:text-slate-900 transition-colors duration-200"
        >
          <ArrowLeft className="w-4 h-4 group-hover:text-slate-600 transition-transform duration-200" strokeWidth={2} />
          Back to Document
        </Link>
      </div>
      ...
      <div className="bg-white/80 backdrop-blur-xl border-2 border-slate-200 rounded-2xl shadow-xl shadow-slate-200/50 p-8 mb-8">
        <div className="text-center space-y-6">
          <div className="inline-flex items-center justify-center w-15 h-15 rounded-2xl bg-linear-to-br from-emerald-100 to-teal-200  mb-6 shadow-lg shadow-slate-200/50 p-4">
      ...
      <div className="space-y-6">
        ...
        {detailedResults.map((result, index) => {
          ...
          return (
            <div
              key={index}
              className="bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-2xl p-4 flex items-center justify-between gap-4 hover:bg-white transition-colors duration-200"
            >
              <div className="flex items-start justify-between gap-4 mb-3">
                <div className="flex-1">
                  <div className="inline-flex items-center gap-2 px-4 py-1 bg-slate-50 border border-slate-200 rounded-lg mb-3">
              ...
              <div className="space-y-3 mb-4">
                {result.options.map((option, optIndex) => {
                  ...
                  return (
                    <div
                      key={optIndex}
                      className={`relative px-4 py-3 rounded-lg border-2 transition-all duration ${isCorrectOption
                    ...
                    >
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <div className="flex items-center gap-2">
                          {isCorrectOption && (
                            <span className="inline-flex items-center gap-1 px-2 py-1 bg-emerald-100 border border-emerald-200 rounded-lg text-xs font-medium text-emerald-600">
                              <CheckCircle2 className="w-3 h-3" strokeWidth={2.5} />
                              Correct
                            </span>
                          )}
                          {isWrongAnswer && (
                            <span className="inline-flex items-center gap-1 px-2 py-1 bg-rose-100 border border-rose-200 rounded-lg text-xs font-medium text-rose-600">
              ...
              {/* Explanation */}
              {result.explanation && (
                <div className="p-4 bg-linear-to-r from-slate-50 to-slate-100/50 border border-slate rounded-xl">
      ...
      <div className="mt-8 flex justify-center">
        <Link to={`/documents/${quiz.document._id}`}>
          <button className="group relative px-8 h-12 bg-linear-to-r from-emerald-500 text-white font-semibold rounded-xl flex items-center justify-center gap-2 hover:bg-emerald-600 transition-colors duration-200">
            <span className="relative z-10 flex items-center gap-2">
              <ArrowLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform duration-200" strokeWidth={2} />
              Return to Document
            </span>
            <div className="absolute inset-0 bg-linear-to-r from-white/0 via-white/20 to-white/0 rounded-xl opacity-0 group-hover:opacity-100 transition-transform duration-200" />
          </button>
        </Link>
      </div>
    </div>
